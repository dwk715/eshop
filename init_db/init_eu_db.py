...
game = {
    ...
    "nsuid": None,  # nsuid --> int 游戏ID,不同服务器同一游戏不同nsuid,根据nsuid查询价格
...
def getGamesEU():
    ...
    for game_info in result:
        ...
        nsuid = int(game_info['nsuid_txt'][0]) if game_info.__contains__('nsuid_txt') else None
        ...
        while (offset < len(nsuids)):
            ...
            for price in response['prices']:
                if price.__contains__('discount_price'):
                    ...
                    game_eu_collection.find_one_and_update({'nsuid': price['title_id']}, {"$addToSet": {
